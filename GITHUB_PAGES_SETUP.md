# KOSPI200 대시보드 GitHub Pages 설치

이 폴더의 파일을 GitHub 저장소 루트에 올리면 대시보드가 GitHub Pages로 배포됩니다.
...
5. **Actions → Refresh KOSPI200 dashboard → Run workflow**에서 `all`을 한 번 실행합니다.
6. 배포가 끝나면 Actions 로그의 Pages URL로 접속합니다.
...
GitHub Actions의 예약 작업은 장시간 지연될 수 있으므로 12:00·15:40에 정확히 실행된다고 보장되지는 않습니다. 실행 결과는 `valuation_data.json`, `news_data.json`에 커밋되고 Pages에 반영됩니다.
